// https://raw.githubusercontent.com/project-serum/serum-dex-ui/master/src/wallet-adapters/ledger/core.ts
import Transport from '@ledgerhq/hw-transport'
import { Transaction, PublicKey } from '@solana/web3.js'
import { Buffer } from 'buffer'
import debug from '@/util/log'
const log = debug.extend('util:wallets:ledger-core')
...
async function ledgerSend(
  ...
  if (payload.length > MAX_PAYLOAD) {
    while (payload.length - payloadOffset > MAX_PAYLOAD) {
      ...
      log('send', (p2 | P2_MORE).toString(16), chunk.length.toString(16), chunk)
  ...
  log('send', p2.toString(16), chunk.length.toString(16), chunk)
...
export async function signTransaction(
  ...
  derivationPath = getSolanaDerivationPath() //Buffer
) {
